...
export class Kudra {
  ...
  constructor(moduleThis: ModuleThis, moduleOptions: KudraOptions) {
    ...
    this.setupDir();
    this.setupLoaders([GlobalLoader, ComponentLoader, PluginLoader, ExperimentalLoader]);
    this.setupHooks();
   ...
   */
  private setupLoaders(loaders: LoaderClass[]) {
    ...
    afterAll.forEach((loader) => createLoader(loader));
  }
